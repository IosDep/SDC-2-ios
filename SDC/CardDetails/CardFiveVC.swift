...
class CardFiveVC: UIViewController {
                    ...
                    if jsonObj != nil {
                        ...
                        if let status = jsonObj!["status"] as? Int {
                            ...
                            if status == 200 {
                                ...
                                self.jobTitle.text = self.handleNillData(value: occupation ?? "")
                                
                                
                                let clientCatDesc = data!["clientCatDesc"] as? String
                                self.investorClassification.text = self.handleNillData(value: clientCatDesc ?? "")
                                ...
                                self.country.text = self.handleNillData(value: Country ?? "")
                                
                                
                                let City = data!["resCity"] as? String
                                self.city.text = self.handleNillData(value: City ?? "")
                                ...
                                self.buldingNo.text = self.handleNillData(value: resBuildingNo ?? "")
                                ...
                                self.area.text = self.handleNillData(value: resArea ?? "")
    ...
    func getInvInfoPDF() {
        ...
        let endpoint = URL(string:APIConfig.GetInvestorInfoPDF)
        let param: [String: Any] = [
            "sessionId" : Helper.shared.getUserSeassion() ?? "" ,
            "lang": MOLHLanguage.isRTLLanguage() ? "ar": "en"
